#!/usr/bin/env python3
"""Anneal port columns for the RAIL controller layout (llm_eval_rail)."""
import sys, os, random, json, math
HERE = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, HERE)
from llm_eval import PORTS
...
def main(seed, iters, span):
    ...
    cur = repair(BASE if seed == 0 else
                 {p: BASE[p] + rng.randint(-30, span) for p in PORTS})
    c, r = total(cur)
    while c is None:
        cur = repair({p: BASE[p] + rng.randint(-30, span) for p in PORTS})
        c, r = total(cur)
    best = (c, dict(cur), r)
    ...
    print(json.dumps({'seed': seed, 'box': best[0], 'cols': best[1],
                      'r': best[2]}))
