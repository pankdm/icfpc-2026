"""LLLM interpreter op-stream, v5 -- v4 plus a SINGLE static render.
...
def build():
    ...
    def round_body(b):
        # render FIRST.  v2 emitted `emit_render` twice -- once for the initial
        # frame and once at the end of each round -- for the same frame stream a
        # single copy at the TOP of the round produces: iteration i paints the
        # state after round i-1, and the run ends when `ri` finds no input, one
        # render after the last round.  Five static `cmd` ops and ~150 emitted
        # ops disappear, and `cmd` is a narrow-band op that costs a row each.
        emit_render(b)
        b.e('ri'); b.SA('kc')
