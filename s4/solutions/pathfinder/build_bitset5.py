#!/usr/bin/env python3
"""Pathfinder bitset BFS v5: literal divisor, fused offsets, minimal reads.
...
def emit_walk(f, prefix, delta, next_label):
    """Walk candidate: cell rec in registers, reads = NEI-store path only.

    Predicate chain identical to build_fifo.emit_walk_choice: rec>0, then
    rec>>8 == EPOCH, then rec&255 == DIST, first match (UP/RIGHT/DOWN/LEFT)
    moves.  NEI is stored for MOVE.
    """
    test = f"{prefix}_TEST"
    epoch_ok = f"{prefix}_EPOCH"
    ...
    f.br(epoch_ok, next_label, next_label)         # rec==0/walls -> next
    f.at(epoch_ok)
    f.e("sp", "M")                                 # [rec]; B=rec
    f.const(8).e("W", "}")                         # A=rec>>8
    f.e("M").load(EPOCH).e("-")                    # read 2; A=EPOCH-hi
    f.br(f"{prefix}_DRAIN", choose, f"{prefix}_DRAIN")
    f.at(f"{prefix}_DRAIN").e("rp").go(next_label)
    f.at(choose)                                   # hi == EPOCH
    f.load(EPOCH).e("M").const(8).e("W", "{")      # read 3; A=EPOCH<<8
    f.e("M", "rp", "-")                            # A=rec-E8=low; []
    f.e("M").load(DIST).e("W", "-")                # read 4; A=low-DIST
...
def build_flow():
    ...
    # Walk-back: same predicates/order as build_fifo (exact tie-breaking
    # parity), but each candidate carries rec in registers: 2 scalar reads
    # (EPOCH, DIST) + 1 cell read instead of fifo's 6-7 scalar reads.
    f.at("BFS_DONE").go("WALK")
    f.at("WALK").load(ROBOT).store(CUR)
    f.cell_loadv()                                 # A=rec (addr was ROBOT)
    f.e("M").const(8).e("W", "}")                  # A=rec>>8 (=EPOCH here)
    f.e("M").const(8).e("W", "{")                  # A=hi<<8
    f.e("sp").load(CUR).cell_loadv()               # re-read rec; [hi8]
    f.e("M", "rp", "W", "-")                       # A=rec-hi8=dist(robot)
    f.e("M").const(1).e("W", "-").store(DIST)      # DIST=dist-1
    f.go("WALK_UP_TEST")
