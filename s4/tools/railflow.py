...
def _lay_once(flow, labels, cols, glyphs, bands, x0, y0, nrail, opmax,
    ...
    for label_i, label in enumerate(labels):
        ...
        if term is None:
            ...
            if cursor.y == entry[label]:
                # a single-row block's last code row is its entry row, whose
                # eastbound man would be caught by the north exit's U-turn
                cursor.newline()
            code_row = cursor.y
...
def _allocate(items, entry, x0, nrail, padded):
...
def lay_cfg_rail(program, flow, port_spec, code_x=30, x0=0, y0=0, op_slack=6,
    ...
    for _ in range(120):
        cursor, entry, items, intent = _lay_once(
            ...
            lit_forbid, pad_after=padded)
        cells, blocked = _allocate(items, entry, x0, nrail, padded)
