...
def build(save):
    ...
    # RELOCATION: the input room used to sit LEFT of C (cols 0-2) with two
    # pipe columns (3-4), so C could never start before column 5 -- which is
    # what makes a 3-row C one interior column short of its 28 cells. Move the
    # room into the RIGHT margin and C starts at column 0. Precedent:
    # reverse-a-list manual-11x11-tuned stacks I/O in the margin with 2-cell
    # L pipes (a room CORNER counts as the source's backward neighbour, and a
    # pipe may turn at the very next cell).
    cx = 0
    block(p, cx, 11, C_W, C_H, C_CELLS, C_MAN)
    ce = cx + C_W - 1                       # C's east wall
    # input room in the right margin, feeding C's east wall with a 2-cell pipe
    p.input_room(ce + 3, 13)
    p.put(ce + 2, 14, '<'); p.put(ce + 1, 14, '<')
    # C -> M: leave C eastward, climb the margin in col ce+1, then west into
    # M's EAST wall. M's bottom wall is unreachable now (C's top wall is
    # directly beneath it), and col ce+2 rows 8-9 already carry the P->O pipe,
    # so the climb must dodge into ce+1 to avoid touching it -- two adjacent
    # pipes parse as one.
    p.put(ce + 1, 12, '>'); p.put(ce + 2, 12, '^')
    p.put(ce + 2, 11, '<'); p.put(ce + 1, 11, '^')
    p.put(ce + 1, 10, '<'); p.put(ce, 10, '<')
