...
def build_encoding(table_budget=None, extra_pair_count=0, tail_constants=False,
    ...
    GB_ROWS = int(os.environ.get("GB_ROWS", "4"))
    nB = (
        -(-len(pair_vals) // GB_ROWS)
        if tail_constants
        else -(-(len(pair_vals) + 1) // GB_ROWS)  # +1 for the zero sentinel
    )
    grid = [[None] * nB for _ in range(GB_ROWS)]
    # widest chunk goes to the physically-last slot; sentinel to slot 0
    chunks_desc = [order[i * GB_ROWS:(i + 1) * GB_ROWS] for i in range(nB)]
    widths = []
    ...
    cellgrid = [[None] * nB for _ in range(GB_ROWS)]
    fill = []
    for j, chunk in enumerate(chunks_desc):
        pj = phys.index(j)
