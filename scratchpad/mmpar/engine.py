...
sys.path.insert(0, os.path.join(_REPO, 'solutions', 'matmul'))
...
def build(ap_rect=None, br_rect=None, verbose=False, ports=False):
    if ap_rect is None:
    ...
    unres(*list(resv))
    if ports:
        globals()['PORTS'] = {'IN': A(spl, 'IN'), 'OUT': A(acc, 'OUT')}
    return g, n_ap, n_br


if __name__ == '__main__':
    g, n_ap, n_br = build(ports=bool(os.environ.get('PORTS')))
    txt = g.render()
    w, h, box = g.footprint()
