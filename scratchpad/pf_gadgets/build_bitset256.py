#!/usr/bin/env python3
"""PF GADGET 1: the 256-bit set as FOUR 64-bit registers (4 parked men holding B).

Provenance: verbatim port of scratchpad/snake_gadgets/build_occ4.py (snake's
proven occupancy gadget); only the output filename changed.  Kept as its own
file so the pathfinder bring-up is self-contained.

For pathfinder this single gadget serves as WALLS *and* VISITED at once: keep
one "blocked" word set (= walls | visited), and SET(n) returns non-zero exactly
when n was neither a wall nor already visited -- i.e. the whole BFS neighbour
test is ONE transaction, not two lookups.
...
path = os.path.join(HERE, 'bitset256.man')
p.save(path)
print(p.render())
