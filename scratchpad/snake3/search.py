"""Snake knob search, adapted from scratchpad/snake2/search4.py for this checkout.

Coordinate descent + random multi-knob jumps.  Every candidate must pass
  (a) all 5 public cases via grade_fast (Rust),
  (b) a 53-case QUICK stress subset (all four wall deaths, both corners,
      grow-50, self-hit, long-lap) -- the public cases exercise ONE death arm
      only, so an ungated sweep silently breaks north/west deaths,
and it is kept only if the score is strictly lower.

Usage: python3 search.py <rounds> <seed> [statefile] [builder]
"""
...
BUILDER = sys.argv[4] if len(sys.argv) > 4 else REPO + "/solutions/snake/build_fold5.py"
MANOUT = STATE.replace(".json", ".man")

# CXL moves the controller's LEFT wall east; the four death highways live in the
# west and must move with it or they fall outside the room.
...
SPREAD = {'ST_OUT': 4, 'ST_IN': 4, 'ST_X0': 4, 'ST_W': 4, 'FEED_W': 5,
          ...
          'DRV_OUT': 4, 'WRAP_E': 4, 'DRVX': 2, 'DISX': 2, 'CY0': 2, 'CXL': 3}
...
def _mk(base, k, v):
    """Set knob k, carrying the coupled columns along."""
    p = dict(base)
    if k == 'CXL':
        d = v - p.get('CXL', 0)
        for kk in LSHIFT:
            ...
            for _ in range(500):
                ...
                for kk in rng.sample(list(SPREAD), rng.randint(1, 4)):
                    if kk in p:
                        if kk == 'CXL':
                            p = _mk(p, 'CXL',
                                    max(0, p['CXL'] + rng.randint(-SPREAD[kk], SPREAD[kk])))
                        else:
