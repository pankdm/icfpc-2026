...
def main():
    ...
    for rnd in range(ROUNDS):
        improved = False
        # --- CW shift move: CW is the box's WIDTH knob (width = CW+21) and every
        # column knob east of the controller has to move with it, so it can never
        # be a single-knob coordinate move.  Sweeping it as a rigid translation is
        # what lets the search trade controller rows (free: height has slack) for
        # box width (not free: width is what the box is).
        cands = []
        for d in range(-8, 5):
            if d == 0: continue
            p2 = dict(best); p2['CW'] = best['CW'] + d
            for kk in SHIFT:
                if kk in p2: p2[kk] += d
            cands.append(p2)
        for res in pool.imap_unordered(_work, cands):
            if res and res[0] < best_score:
                ...
                print("  cw %-3s score %d box %d ticks %.0f" %
                      (res[4]['CW'], res[0], res[1], res[2]), flush=True)
        # --- coordinate descent ---
        ...
        for wave in range(30):
            ...
            for _ in range(500):
                p = dict(best)
                if rng.random() < 0.45:
                    d = rng.choice([-3, -2, -2, -1, -1, 1, 2])
                    p['CW'] += d
                    for kk in SHIFT:
                        if kk in p:
                            p[kk] += d
                for kk in rng.sample(list(SPREAD), rng.randint(1, 7)):
                    if kk in p:
