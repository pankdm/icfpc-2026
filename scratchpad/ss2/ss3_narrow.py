...
cols 440-448, 449x386 -> 440x386, box 201601 -> 193600 (1.041x).
...
SRC = os.path.join(HERE, "ss3-base.man")
DST = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, "ss3-narrow.man")
...
JOBS = [
    ...
    ([(x, 0) for x in range(440, 445)]
     + [(444, y) for y in range(1, 30)]
     + [(x, 29) for x in range(443, 439, -1)],
     comb(439, 438, 1, 28, 5)),
    ([(x, 119) for x in range(440, 449)]
     + [(448, y) for y in range(120, 149)]
     + [(x, 148) for x in range(447, 439, -1)],
     comb(439, 438, 120, 147, 9)),
]
...
def main():
    ...
    for new, tail in ((JOBS[0][1], (439, 29)), (JOBS[1][1], (439, 148))):
