...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.dirname(
    os.path.dirname(os.path.abspath(__file__)))), "solutions", "sudoku-validity"))
from littleman import Program
import serp

# ------------------------------------------------------------------ op streams
# bit = idx + 9*(v-1);  shift1 = 9*(7-v) - idx;  w' = 9*(7-v) is broadcast value 5
# THREE broadcasts, not five.  `decide` -- the latency the timer ring has to cover
# and therefore the round period and therefore the whole tick count -- is
#     avail[last] + (room ops after the last read) + 2 pipe + 6 strip,
# and avail[last] is the DISPATCH's span from its first broadcast to its last.
# Broadcasting r/3 and c/3 (which only BOX wants) costs 6 dispatch ops inside that
# span for nothing: BOX is *parked* on its `r` until w arrives anyway, so its
# spare time is free and the dispatch's is not.  Moving the box-index arithmetic
# into BOX takes the span from 16 ops to 10.
#
# BOX derives idx without a third register by folding the two divisions into one:
#     3*(r/3) + c/3 == (9*(r/3) + c) / 3          (9*(r/3) is a multiple of 3)
# so it divides r by 3, scales by 9, adds c, and divides by 3 again.
DISPATCH9 = list("rSrSrM7-M9*S")                   # r, c, 9*(7-v)
TAIL = list("-M1{s1N~M1{s")                        # shift1 -> both lane masks
ROW9 = list("rMrr") + TAIL                         # idx = value0 (r)
COL9 = list("rrMr") + TAIL                         # idx = value1 (c)
BOX9 = list("3Mr/M9*Mr+M3W/Mr") + TAIL            # idx = (9*(r/3) + c) / 3

# 6-row strip (was 7).  The 7-row version had to send the INACTIVE lane's man all
# the way round the bottom so that it re-entered the shared `X` travelling NORTH
# while the duplicate entered it travelling WEST.  Testing BP (`d`, "is this lane
# active") BEFORE A (`X`, "was the bit free") does the same discrimination one row
# higher: the inactive man goes round the bottom and arrives at `X` from the south,
# the active man is deflected west into `X` one row earlier, and `X`'s "continue
# straight" then means north for the first and west (-> report) for the second.
#            col0 report | col1 up-lane | col2 down-lane
STRIP3 = [" >v", " Mr", "H~b", "s &", "^Xd", " ^<"]
STRIP_DECIDE = 6            # ticks from a strip's `r` to the `s` that emits 0
...
# --------------------------------------------------------------------- layout
def build(DW=21, DH=3, RW=8, BW=10, H=5, timer_left=1, timer_lap=0, DXfix=0,
    ...
    for cb in cands["box"]:
        for cr in cands["row"]:
            for cc in cands["col"]:
                ...
                if not ok:
                    continue
                # `@ 1 N M` sits on the fork row itself and the first Y is at
                # cols[0]-1, so the leftmost strip needs cols[0] >= 6.
                decide = max(pick[n][2][-1] for n in pick) + 2 + STRIP_DECIDE
                lap = timer_lap or (decide + 1 + lap_slack)
                if lap % 2:                       # lap = 2*(E-L+1) is even
                    lap += 1
                # the fork chain's survivor drops into the timer at E = cols[-1]+1,
                # which IS the ring's east column; the ring's WEST column then sets
                # the lap, so cols[-1] is free to be as small as the strips allow.
                so = solve_out(scols, 1, MW - 2, 6, min_last=lap // 2 - 1)
                if not so:
                    continue
                cols = sorted(so[0])
                ...
                if not (DX < min(si) and max(si) < DX + DW + 1):
                    continue
                width = max(MW, DX + DW + 5, cols[-1] + OUTW)
                key = (max(width, HGT) ** 2 * lap, width, decide)
                if best is None or key < best[0]:
                    best = (key, pick, scols, cols, so[1], si, lap)
    assert best, "no merged geometry satisfies both bindings"
    _, pick, scols, cols, lanepipe, inpipe, timer_lap = best
    assert timer_lap % 2 == 0, timer_lap
    ...
    # this costs 3 (2 pipe columns are reused by the room) and lets DX be 0, which
    # is what keeps DW at 23 -- the width at which the whole 20-op dispatch stream
    # fits on serpentine row 1 and `avail` carries no turn-cell tax.
    assert DX == 0, ("east-side INPUT assumes the dispatch starts at x=0", DX)
    p.input_room(DW + 2, 3)
    ...
    srcy = YA + H + 3
    YG = srcy + 2
    E = cols[-1] + 1                      # ring's east column = the drop column
    L = E + 1 - timer_lap // 2            # ring's west column sets the lap
    GW = E + 2
    p.room(0, YG, GW, 6 + 6)              # 6 strip rows + fork/return + timer + walls
    R0, R1, R2 = YG + 1, YG + 2, YG + 3
    # The B:=-1 init now sits ON the fork row (`@ 1 N M` at columns 1..4) instead of
    # in a 3-column vertical tail: the 6-row strip's `H` lands exactly where that
    # tail used to turn north, and the flat version also frees R0 completely.
    assert cols[0] >= 6, ("no room for `@ 1 N M` west of the first Y", cols)
    p.put(1, R1, "@")
    for k, ch in enumerate("1NM"):
        p.put(2 + k, R1, ch)

    # every strip is forked, including the last: the survivor of the chain walks on
    # east and drops down column E straight into the timer ring's east turn, so no
    # separate timer fork column is needed and lap is pinned to 2*(E-L+1).
    ...
    assert L + 3 < E, ("timer geometry", L, E)
    assert 2 * (E - L + 1) == timer_lap, ("lap mismatch", E, L, timer_lap)
    p.put(L, ty, ">"); p.put(E, ty, "v")
    p.put(E, ty + 1, "<"); p.put(L, ty + 1, "^")
    p.put(L + 3, ty + 1, "1"); p.put(L + 2, ty + 1, "s")

    # ---- O beside the gadget, in the gadget's own rows.  Entering O through its
    # TOP wall instead of its west wall buys two columns: a west-wall entry needs
    # two pipe cells BETWEEN the two rooms (GW, GW+1), so O starts at GW+2; a top
    # entry spends the same cells on rows instead and O starts at GW.
    p.pipe([(GW, ty - 2), (GW + 1, ty - 2), (GW + 1, ty - 1)])
    p.output_room(GW, ty)
    ...
    for n in ("box", "row", "col"):
        ...
        for i, c in enumerate(ck["ops"][n]):
            ...
            assert d[0][1] == ck["inpipe"][n] and d[0][0] < d[1][0], \
                (n, "r", u, "binds", d[:2])
    forks = sorted(c - 1 for c in cols)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--dw", type=int, default=21)
    ...
    p, ck = build(DW=a.dw, DXfix=a.dx, RW=a.rw, BW=a.bw, H=a.rh,
                  ...
                  lap_slack=a.slack, HGT=a.hgt, OUTW=a.outw)
    check(ck)
    out = a.out if a.out.startswith("/") else os.path.join(os.path.dirname(os.path.abspath(__file__)), a.out)
    p.save(out)
