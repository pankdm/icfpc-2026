...
def dispatch(g, c):
    # c heading E at (2,DISPATCH_Y).  B=op.  op==1 -> GET (straight); else -> stub.
    c.gto(18).e('1-X')        # glide east of the col1-15 approach corridor, then test
    xc = (c.x - 1, c.y)
    g.C(xc[0], xc[1] - 1, 'H')   # stub for op>=2 (off-corridor)
    get_op(g, Cur(g, c.x, c.y, 'E'))
