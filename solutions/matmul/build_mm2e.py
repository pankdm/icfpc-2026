...
AP_EXIT = col(2, 21, 28) + row(28, 2, 11)
BR_LEAD = col(-8, 44, 48) + row(48, -8, -1) + col(-1, 46, 48) + [(0, 46)]
BR_EXIT = col(0, 57, 58) + row(58, 0, 20) + col(20, 40, 58) + row(40, 15, 20)

CORR = {
    'IN':  col(13, 0, 1),
    'AR':  row(28, 22, 23) + col(23, 28, 34) + row(34, 15, 23) + col(15, 34, 35),
    'BF':  row(37, 10, 11) + col(10, 37, 45) + row(45, -1, 10) + col(-1, 44, 45),
    'SD':  row(11, 28, 29) + col(29, -4, 11) + row(-4, -14, 29) + col(-14, -4, 38) +
           row(38, -14, -12),
    'PP':  row(37, 20, 21) + col(21, 35, 37) + row(35, 21, 48) + col(48, 35, 41) +
           row(41, 40, 48),
    'OUT': row(38, 40, 43),
    'CF':  row(38, 21, 23) + col(21, 38, 58) + row(58, 21, 26),
    'CR':  row(56, 22, 23) + col(22, 41, 56) + row(41, 22, 23),
    'CP':  col(20, 12, 13) + row(13, 20, 30) + col(30, 6, 13) + [(31, 6)],
    'CTL': col(38, 12, 13) + row(13, 31, 49) + col(49, 13, 53) + row(53, 33, 49) +
           col(33, 52, 53),
}


def build(ap_rect=(2, 2, 16, 20, False), br_rect=(0, 46, 20, 12), verbose=False):
    rt = RT.Router()
    g = RGrid(rt)
    ...
    arel = R.arel(g, 12, 24)
    arel.attach('AP', 'L', 28, 'in')
    arel.attach('AR', 'R', 28, 'out')
    mul = R.mul(g, 12, 36)
    crel = R.crel(g, 24, 54)
    crel.attach('CF', 'B', 26, 'in')
    crel.attach('CR', 'L', 56, 'out')
    acc = R.acc(g, 24, 36)
    acc.attach('CTL', 'B', 33, 'in')
    rt.add_output_room(44, 37)
