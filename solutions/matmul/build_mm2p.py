...
AP_LEAD = row(5, 10, 11) + col(10, 2, 5) + row(2, 3, 10)


def ap_exit(rect):
    """Corridor from the A-band snake's tail down to AREL's AP attachment (11,26).

    The tail column/row follow from the snake shape, so this must be derived, not
    hardcoded -- the old constant pinned the band to exactly 15x18.
    """
    x0, y0, w, h = rect[:4]
    lo = x0 - w + 1
    # snake starts at (x0,y0) heading west; row r ends at lo when r is even
    tx = lo if (h - 1) % 2 == 0 else x0
    ty = y0 + h - 1
    return col(tx, ty, 26) + row(26, tx, 11)


AP_EXIT = ap_exit((2, 2, 15, 18))
BR_LEAD = col(-8, 42, 46) + row(46, -8, -1) + col(-1, 44, 46) + [(0, 44)]


def br_exit(rect):
    """Corridor from the B-ring snake's tail to MUL's BR attachment.

    Derived from the snake shape (it runs east-first from (x0,y0)), so the B ring
    can be resized -- the old constant pinned it to exactly 20x12.
    """
    x0, y0, w, h = rect[:4]
    tx = (x0 + w - 1) if (h - 1) % 2 == 0 else x0
    ty = y0 + h - 1
    return col(tx, ty, 56) + row(56, min(tx, 20), max(tx, 20)) + \
        col(20, 38, 56) + row(38, 15, 20)


BR_EXIT = br_exit((0, 44, 20, 12))

CORR = {
    'IN':  col(13, 0, 1),
    'AR':  row(26, 22, 23) + col(23, 26, 32) + row(32, 15, 23) + col(15, 32, 33),
...
# MEASURED ring floors (scratchpad/mm2/apfloor.py, ringsweep.py), not derived:
#   A queue  AP >= 254 pipe cells  (238 deadlocks 16x16x16; AREL + the AR pipe buffer
#            the rest of N*M = 256, which is why 298 was 17% of slack)
#   B ring   BR >= 240 pipe cells  (220 deadlocks 16x16x16)
# 14x16 / 18x10 sit just above both.  DESIGN_mm2's "L ~ 1.15*V => ~300" was derived.
def build(ap_rect=(2, 2, 14, 16, False), br_rect=(0, 44, 18, 10), verbose=False):
    rt = RT.Router()
    g = RGrid(rt)
    ...
    arel = R.arel(g, 12, 22)
    arel.attach('AP', 'L', 26, 'in')
    arel.attach('AR', 'R', 26, 'out')
    mul = R.mul(g, 12, 34)
    crel = R.crel(g, 24, 52)
    ...
    n_br = route_long(g, A(brel, 'BR'), A(mul, 'BR'), br_rect, BOUND, end_direction='N',
                      lead_avoid=outside(BR_LEAD), exit_avoid=outside(br_exit(br_rect)))
    for c in list(resv):
        if g.get(*c) == '\x02':
            del g.c[c]
