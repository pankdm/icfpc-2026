...
def build(stage="drainA", W=31, H=42):
    ...
    if stage in ("run","full"):
        ...
        # HEIGHT LEVER (opt2): SB serpentine height is the SOLE box driver (tallest ring).
        #   Deadlock floor is H=33 (fails 16^3); H=34 is the exact minimum that passes the
        #   max-capacity case (272 live values). H=35 keeps 1 full serpentine row of slack
        #   above that floor -> height 90 (box 8100) vs 92 (box 8464), avgTicks unchanged.
        folded([11,10,9,8,7], [15,14,13,12], 35, -39)
    else:
        big = -145; scy=-24
...
if __name__=="__main__":
    ...
    if stage in ("run","full"):
        out = compact(out)
        open(os.path.join(os.path.dirname(__file__),"matmul-opt2.man"),"w").write(out)
    print(out)
