...
# ---- BESIDE-HANG helpers (ported from build_opt5 hang4; proven on the oracle) ----
...
def _vser_up(cols, top, bot):
    """vertical serpentine entered at (cols[0],bot) going UP; ends at (cols[-1],top) for odd len."""
    ...
    return pts, lvl
def hang4(P, af, ar, feed_body, ret_body, top, bot, RL, RY, ef, er, rrcol=None):
    """Hang one ring BESIDE the CTRL. feed_body/ret_body serpentine at rows top..bot (beside
    CTRL -> add NO height). Relay at (RL,RY). feed & return leads run BELOW the bodies
    (rows bot+1,bot+2). af/ar = CTRL top-wall attach cols. ef/er = escape rows (negative)."""
    LRf=bot+1; LRr=bot+2
    _relay(P, RL, RY)
    RR=RL+6 if rrcol is None else rrcol
    feed_in=RL+4
    fp=[(af,-1),(af,ef),(feed_body[0],ef)]
    sp,lvl=_vser(feed_body, top, bot); fp+=sp
    fx=feed_body[-1]
    fp+=[(fx,LRf),(RR,LRf),(RR,RY-2),(feed_in,RY-2),(feed_in,RY-1)]
    P.pipeC(fp)
    rp=[(RL+2,RY+4),(RL+2,LRr),(ret_body[0],LRr)]
    sp2,lvl2=_vser_up(ret_body, top, bot); rp+=sp2
    rx=ret_body[-1]
    rp+=[(rx,er),(ar,er),(ar,-1)]
    P.pipeC(rp)
...
def build(stage="tiny"):
    ...
    if stage=="fold":
        ...
        # SC's relay(-14) so SB's pipe clears the shallow relays (the leg-crossing fix).
        hang4(P, SBf, SBr, [57,58,59,60,61], [69,70,71,72,73], 1, 62,
              RL=63, RY=25, ef=-15, er=-16, rrcol=62)
    else:
