#!/usr/bin/env python3
"""memory rewind v3 -- PROTOCOL CHANGE: op before value, no value on reads.
...
# purely by column:
#     CMD/P2 midpoint 5   : `r` at x<=4 reads COMMANDS, at x>=6 reads the BELT
#     OUT/P1 midpoint 8.5 : every `s` must sit at x>=9 to reach the belt
...
def build():
    ...
    # 5 sends/lap keeps every init 's' at x>=9 (the OUT/P1 midpoint is 8.5).
    ...
    for y in range(8, 14):
        P(3, y, ' ')
    P(3, 14, '>')
    P(4, 14, 'r')                          # new value    (CMD 3 vs P2 5)
    P(5, 14, 'M')                          # B = value
    P(6, 14, 'r')                          # old value, discarded (P2 3 vs CMD 5)
    P(7, 14, 'W')                          # A = value
    P(8, 14, ' ')
    P(9, 14, 's')                          # -> belt      (P1 5 vs OUT 6)
    ...
    # ================= CONTROL : 10x8, cols 0-9, rows 33-40 =================
    CX, CY = 0, 33
    ...
    # ================= IO =================
    p.output_room(2, 20)                   # under OUT's column (x=3)
    p.input_room(12, 33)                   # east of CONTROL's new right wall

    # ================= pipes =================
    out = [(X_OUT, PIPE_ROW), (X_OUT, PIPE_ROW + 1)]
    cmd = [(X_CMD, 32), (X_CMD, PIPE_ROW)]
    ipipe = [(11, 34), (10, 34)]           # input room left wall -> CONTROL col 9
...
if __name__ == '__main__':
    prog = build()
    out = os.path.join(os.path.dirname(__file__), 'rewind-v3.man')
    prog.save(out)
