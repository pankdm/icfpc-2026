...
Width 23 is blocked by the belt, and by one rule this build measured:
    ...
    A PIPE MAY NOT RUN FLUSH ALONGSIDE THE ROOM IT TERMINATES AT.
    ...
    "pipe loops back to the room it started from".  Flush along HOP's east wall
    (col 23) and CONTROL's east wall (col 13) is FINE and v13 has always done
    it.  So the column beside MEM is permanently dead, and MEM effectively
    costs MEM_W + 1 columns.

Budget for p2 (which needs >= 99 cells) at width 23, height 23, i.e. no row-23
corridor below HOP and no descent column east of it:
    MEM 18 wide -> dead col 18 -> snake cols 19-21 rows 0-14 (45) + descent
    col 22 (23) + isthmus col 13 (8) + the row-14 approach (~10)  =  ~82.
    MEM 17 wide -> ~91 but col 17 is then dead too, so it buys nothing.
Short by ~17 cells in every arrangement tried.  The two ways out are both
