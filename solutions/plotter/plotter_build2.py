...
BL, BR = 8, 48             # op-band columns (junction at BL-1=7); band width 41
BIN = BL                   # belt-in attach at band WEST end (=11)
BOUT = BR                  # belt-out attach at band EAST end (=51)
CMD_S = BR + 2             # cmd send excursion col (picks cmd over belt-out)
CCMD = BR + 3              # cmd attach (outgoing) -- just east of the band
GL = 1                     # gate interior left col
GR = CCMD + 1              # gate interior right col
...
if __name__ == "__main__":
    import json
    p = build()
    path = os.path.join(os.path.dirname(__file__), "plotter-folded.man")
    p.save(path)
