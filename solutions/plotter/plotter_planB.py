...
RING = os.environ.get("RING", "boxeq")
...
CINP = 3
INP_R = 5
BL, BR = 8, int(os.environ.get("BR", "48"))
BIN = BL
BOUT = BR
CMD_S = BR + 2
CCMD = BR + 3
...
class BTurtle:
    ...
    return-glide rows). West rows hold ops in reversed stream order. Uses despine
    (single-char ops only) so reversal is a no-op per cell. Block boundaries and
    excursions flush to a fresh east-row start so rails/excursions stay east-oriented."""
    def __init__(self, p, y):
        self.p = p
        self.y = y
        self.x = BL
    ...
    def lit(self, k):                      # despine mode never calls this
        for ch in "`" + str(k) + "`":
            self.op(ch)
...
def build():
    ...
    put(p, BL - 2, T, "@")
    t = BTurtle(p, T) if os.environ.get("MODE") == "boustro" else Turtle(p, T)
    ...
    if os.environ.get("BELT") == "selfloop":
        ...
        GBOT = south + D
    elif os.environ.get("RELAY") == "tight":
