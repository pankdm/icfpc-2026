...
def build(save_to=None, CY0=8, CBOT=85, CW=55, CXL=0,
    ...
    face(LOOPR, "E")
    # THE SCAN LOOP, LAID FLAT.  It used to be a 3-column x 5-row rectangle plus an
    # arm row -- SIX rows -- because `d` turns the eastbound man CLOCKWISE, i.e.
    # SOUTH, and the four ops r:B s:B ~ X were then stacked down that column.  Turn
    # him WEST on the first cell below `d` instead and the same four ops lie along
    # ONE row, with the loop closing in two:
    #
    #     ys    : (LOOPR) >  . . . . . . (LOOPX) d          d: BP>0 -> south
    #     ys+1  : (D_COLL) v ... <  X  ~  s  r  (LOOPX) <    walked WESTWARD
    #
    # `~` XORs two display addresses (both 0..255), so A is never negative and X
    # heading WEST has only two live arms: straight (A==0, the collision, which
    # carries on west to the D_COLL drop) and clockwise (A>0, NORTH), which lands
    # back on the `>` that vjump already wrote at (LOOPR, ys).  Putting X exactly on
    # LOOPR is what closes the loop without a third row, so LOOPR is pinned four
    # columns west of LOOPX here rather than being a free knob.
    # `m` (decrement BP) is the loop COUNTER -- `b` loaded BP=K before the loop and
    # `d` runs while BP>0 -- so it cannot be dropped; without it every case times
    # out with no error.  It touches only BP, never A or B, so unlike r/s/~/X it
    # may sit anywhere on the westward run.
    LOOPR = LOOPX - 5
    ys = rows(2)
    vjump(E, LOOPR, ys, "E")
    for (x, y, ch) in [(LOOPX, ys, "d"),
                       (LOOPX, ys + 1, "<"), (LOOPX - 1, ys + 1, "m"),
                       (LOOPX - 2, ys + 1, "r"), (LOOPX - 3, ys + 1, "s"),
                       (LOOPX - 4, ys + 1, "~"), (LOOPR, ys + 1, "X")]:
        L.put(x, y, ch)
    E.ops.append((LOOPX - 2, ys + 1, "r", "B"))
    E.ops.append((LOOPX - 3, ys + 1, "s", "B"))
    arm(LOOPR - 1, ys + 1, D_COLL)                           # collision -> repaint
    E.at(LOOPX + 1, ys, "E")
    yn = rows()
    ...
    face(LOOPR, "E")
    # Same fold as the NOEAT scan loop, one row shallower.  `r:B` used to sit on
    # its own row purely because the man arrives heading SOUTH out of `d` and the
    # `>` that starts the eastward run needed a cell of its own.  Put the `>` on
    # that row instead and let `r:B` be the first cell of the eastward run: the
    # pop, the two `s:D` sends and the turn-down all share ONE row, and the loop
    # closes in THREE rows instead of four.  r:B lands on LOOPX+1, which is still
    # inside the body lane, and _assert_bindings re-derives it from the grid.
    yr = rows(3)
    vjump(E, LOOPR, yr, "E")
    for (x, y, ch) in [(LOOPX, yr, "d"),
                       (LOOPX, yr + 1, ">"), (LOOPX + 1, yr + 1, "r"),
                       (REPD, yr + 1, "s"), (REPD + 1, yr + 1, "9"),
                       (REPD + 2, yr + 1, "s"), (REPD + 3, yr + 1, "v"),
                       (REPD + 3, yr + 2, "<"),
                       (LOOPM, yr + 2, "m"), (LOOPR, yr + 2, "^")]:
        L.put(x, y, ch)
    E.ops.append((LOOPX + 1, yr + 1, "r", "B"))
    E.ops.append((REPD, yr + 1, "s", "D"))
    E.ops.append((REPD + 2, yr + 1, "s", "D"))
    E.at(LOOPX + 1, yr, "E")
    yn = rows()
    vjump(E, LOOPX + 2, yn, "W")
    E.seq(["1", "N", "s:D", "H"])
