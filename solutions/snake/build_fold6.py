...
def build(save_to=None, CY0=8, CBOT=85, CW=55,
    ...
    for key, op in OPKEY.items():
        ...
        sdP = 32 * sdy + sdx                   # step in GUARD-BIT address space
        E.seq(["r:S"] + _lit(sdP) + ["s:S",                   # dP  (replaced)
                                     "r:S", "s:S",            # P
                                     "r:S", "s:S"]            # MASK
              + ["r:S"] + _lit(sda) + ["s:S",                 # da  (replaced)
                                       "r:S", "s:S",          # ha
                                       "r:S", "s:S",          # fa
                                       "r:S", "s:S"])         # K
        ret_dispatch()
...
if __name__ == "__main__":
    ...
    path = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, "fold17.man")
    # fold17: 71x69, box 5,041, avgTicks 6,966, local 35,113,590
    prog, cap, nrows = fit(save_to=path, CW=50, CY0=8, ST_OUT=22, ST_IN=23, ST_X0=19, FEED_W=26, LOOPR=37,
                           D_REP=6, D_HY=7, HW_RET=46, HW_TICK=35, HW_SPAWN=47,
                           HW_DIR=34, D_EAT=30, D_NOEAT=21, DRV_OUT=49, DRVX=4)
    print("saved", path)
