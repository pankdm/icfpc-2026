...
MIN_CAP = 51                         # peak ring occupancy is EXACTLY 50 (fold14 finding)
...
def build(save_to=None, CY0=8, CBOT=85, CW=55, CXL=0,
    ...
    # strip stays at CW..CW+20; only the LEFT wall moves east by CXL.  Columns
    # 0..CXL-1 then hold nothing at all, so the footprint's width is CW+21-CXL.
    # This is the one direction the geometry was never squeezed in: measured on
    # fold14, controller columns 1..16 held 121 of ~700 content cells, because
    # the emitter starts every westward row at IXLO and the state lane's binding
    # window reaches all the way to column 1.  Raising IXLO packs those ops east
    # at the cost of extra wraps -- rows for columns, and `fit()` prices it.
    assert 0 <= CXL, CXL
    ...
    # ═══ INIT ════════════════════════════════════════════════════════════
    y0 = R.take()
    # The man starts on the INIT row, two columns in from the LEFT WALL -- which
    # moves with CXL, so this cannot be the literal column 3.
    _mx = g["IXLO"] + 2
    L.put(_mx, y0, "@")
    E.at(_mx + 1, y0, "E")
