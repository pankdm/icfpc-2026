...
OUT = 'solutions/sort-numbers/select-v4.man'
...
def build_controller(L):
    ...
    # LOAD LOOP row3 — COMPACT (v4): send left at col5 (still nearer FEED than OUTPUT),
    # killing v3's 9-cell read->send glide and the 12-cell row5 loopback glide.
    put(2, 3, '>')            # MLL merge
    put(3, 3, 'r'); put(4, 3, '+')         # A=v ; A=v+bias
    put(5, 3, 's')            # send FEED (col5: nearer FEED path[0](17,9) than OUTPUT(4,21))
    put(6, 3, 'm'); put(7, 3, 'd')         # bp-- ; E: bp>0->CW=S loop, bp==0->E exit
    # loopback (bp>0): turn S then W along row4 back to MLL (short; shares (2,4)^ w/ init)
    put(7, 4, '<'); put(2, 4, '^')
    # exit (bp==0): turn S at col8 (clear of the EMPTY path on col11) ; sentinel -> FEED ;
    # then row7 W -> col1 -> (1,8) highway.
    put(8, 3, 'v')                          # turn S
    put(8, 4, '1'); put(8, 5, 'N'); put(8, 6, 's')         # A=1 -> A=-1 -> send sentinel FEED (col8)
    put(8, 7, '<')                          # turn W on row7 (EMPTY path uses only cols11..13 on row7)
    put(1, 7, 'v')                          # (1,7) -> (1,8) highway
