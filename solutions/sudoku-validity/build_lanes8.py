#!/usr/bin/env python3
"""sudoku-validity: lanes7 + ROW/COL spend BOX's spare height to lose a column each,
and the O room moves beside its pipe instead of below it.
...
from build_lanes2 import DISPATCH_OPS, ROW_OPS as _R, COL_OPS as _C, BOX_OPS
# BOX is H=7 (outer 11x9) while ROW/COL were H=5 (outer 11x7): two rows of band
# height were going spare. W=8,H=7 spends them for one column each -- but W=8
# lands the two lane sends only 2 columns apart and 3-wide strips need 3, so one
# pad before the first `s` slides it from x=4 to x=3 (vs x=6).
ROW_OPS = _R[:17] + [" "] + _R[17:]
COL_OPS = _C[:17] + [" "] + _C[17:]
import serp
...
def build(timer_left=1, depth=0, depth2=0):
    p = Program()
    ...
    # timer forks off a free column: it must not be adjacent to another fork,
    # or its Y overwrites that fork's return `>` and the chain dies.
    ...
    for e, d in ((EXC, depth), (EXC2, depth2)):
        if d:
            p.put(e, ty + 1, "^"); p.put(e, ty + 1 - d, ">")
            p.put(e + 1, ty + 1 - d, "v"); p.put(e + 1, ty + 1, ">")

    # O beside its pipe, not below it: saves two rows for free. A long pipe up
    # into the dispatch band would save five, but output transit delays the round
    # gate ~1:1, costing more ticks than the box gains.
    p.pipe([(18, YG + 13), (18, YG + 14), (19, YG + 14)])
    p.output_room(20, YG + 13)
...
if __name__ == "__main__":
    tot = int(sys.argv[1]) if len(sys.argv) > 1 else 0
    depth, depth2 = min(tot, 8), max(0, tot - 8)
    p, ck = build(depth=depth, depth2=depth2)
    check(ck)
