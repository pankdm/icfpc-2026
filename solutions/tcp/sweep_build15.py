...
from checker_u5 import emit_checker_u5


def build_full15(CB=3, yr=2, gap=2, cy_checker=3,
                 lane_gap=2, drain_row_off=5, seq_row_off=8, out_dx=1):
    L = Layout()
    ...
    hints = emit_checker_u5(L, cx, cy, seq_j=seq_row_off + yr - cy)
    seqW = hints['seqW']                 # (cx-1, row)  west attach, flows EAST
    g0, g1 = REW + 1, cx - 1             # the two gap columns

    # ---- SEQ PIPE: reader east wall -E-> g0 -S-> g1 col -E-> checker west ----
    srow = seqW[1]                       # straight run: `s` row == checker attach row
    place_pipe(L, [(g0, srow), (g1, srow)], exit_dir=DIRS['E'])

    # ---- INPUT: room above the checker, pipe W into the reader's east wall ----
    L.input_room(cx, yr - 2)             # 3x3 above the checker, rows yr-2..yr
    place_pipe(L, [(x, yr) for x in range(cx - 1, REW, -1)], exit_dir=DIRS['W'])
