    ...
    for edge_i in order:
        ...
        for corridor_i, members in enumerate(corridors):
            ...
            for member_i in members:
                ...
                overlap_lo, overlap_hi = max(lo, lo2), min(hi, hi2)
...
def lay_cfg_boustrophedon(
    program, flow, port_spec, code_x=30, x0=0, y0=0, op_slack=6
):
    ...
    for _ in range(40):
        cursor, entry, edges, intent = _lay_once(
            flow, labels, cols, glyphs, bands, x0, y0,
            corridor_count, opmax,
        )
