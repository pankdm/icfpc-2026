...
def lay_cfg_controller(
    ...
    for block_index, (label, tokens) in enumerate(flow.blocks.items()):
        ...
        for token in tokens:
            ...
            if boustrophedon:
                # Keep executing on the wrap row in the opposite direction.
                # All constants emitted by const_ops are direction-independent,
                # and every external operation has an explicit binding zone.
                if token in cols:
